...
let map1_prg: program = [
	new binding("map1",
		new lambda_form([], false, ["f"],
			new letrec_expr([
				new binding("mf", new lambda_form(
					["f", "mf"],
					false,
					["xs"],
					new case_expr("xs",
						new alternatives([
							...
							new algebraic_alt("Cons", ["y", "ys"],
								new let_expr([
									new binding("fy", new lambda_form(["f", "y"], true, [], new application("f", ["y"]))),
									new binding("mfy", new lambda_form(["mf", "ys"], true, [], new application("mf", ["ys"])))
